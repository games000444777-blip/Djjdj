...
class MineEvoAutoMod(loader.Module):
    """Автосбор шахт для @mineEvo бота (с ключами)"""
    ...
    async def genkey(self, message):
        """[дни] — сгенерировать ключ (только owner)"""
        ...
        data = get_keys_data() or {"owner_id": OWNER_ID, "keys": {}}
        data["keys"][key] = {
            "user_id": None,
            "expires": expires,
    ...
    async def _watch_mineevo(self):
        try:
            ...
            # Ловим новые и редактированные сообщения
            self.handlers.append(
                self.client.add_event_handler(process, events.NewMessage(chats=chat_id))
            )
            self.handlers.append(
                self.client.add_event_handler(process, events.MessageEdited(chats=chat_id))
            )
